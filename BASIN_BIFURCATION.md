...
The **seeding** is the candidate explanation, and it is the one axis the two runs do not share. D2 starts each `alpha` from its own iteration-0 tensor, recomputed under `W0 + alpha·ΔW` (the protocol stated above); T1.2 is a continuation sweep that seeds each `alpha` from the previous `alpha`'s settled state. If that is what it is, then at `alpha` = 0.50 two initial states reach two different settled words under **identical** weights. The alternative is that it is a readout artifact rather than genuine bistability: `comrade` sits at rank 2 here, so the margin **0.1144** *is* the `prolet` − `comrade` logit gap (the table's −0.1144). The label here is therefore being read at a near-tie: two tokens 0.11 logits apart decide which basin this row reports. **Do not size that against C-07.** C-07 is a `1−cos` distance between settled state *tensors* and 0.1144 is a *logit margin*; the two are in different units and this repository holds no calibration between them, so quoting the one to bound the other would be a category error. The near-tie stands on its own as the reason a readout artifact cannot be ruled out. (For completeness on C-07's own terms, since the pooled figure is easy to misread: the pooled mean within-basin spread is 3.319e-03 across all five basins against the `Anarch`–`prolet` gap of 2.874e-03, but `prolet`'s *own* spread is 2.773e-03, just below that gap, so the pair in question does resolve in state space — see C-07's decomposition.) The two runs are not a matched pair, which is why the row is `provisional` and not `supported`.
...
superseded reading; what is measured is a class change, which is real and is ladder step 3]**. At alpha = 1.50 the basin tips into `Divine` and the lag-1 cosine **collapses** from ~1.0000 (a fixed point) to 0.7340 — the fixed point gives way to a period-2 trajectory whose readout is `Divine`. So the `alpha`-axis reads `prolet` (fixed point) → `comrade` (fixed point) → `Divine` (period-2): two transitions along one line — the first a relabelling of the readout while the settled branch moves smoothly (ladder step 2, and no longer pending: T1.1 established it, C-26 `retired`, C-56 `supported`, with C-68 the open discrepancy at `alpha` = 0.50), the second a genuine change of dynamical class (step 3, C-28, `provisional`). That the period-2 trajectory is the **same** orbit `Divine` holds in the frozen baseline is **not established here**: this sweep records a readout label and a lag-1 collapse, and no committed artifact compares the two trajectories' states. Orbit identity is consistent with these numbers, not shown by them — substituting a readout label for state identity is exactly what retired C-26.
