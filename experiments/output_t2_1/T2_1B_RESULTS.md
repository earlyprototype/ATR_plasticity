...
- **connected loop**: settles into the genuine period-2 `Divine` cycle (lag-1 0.685,
  lag-2 1.000);
- **feedback severed**: settles onto a **fixed point** (lag-1 0.998) at `【`, a token
  belonging to none of the five baseline basin families.
...
admissible observation in the project of feedback changing the behavioural outcome, and
it changes the **dynamical class**, not merely the label: with feedback the system finds
the pre-existing `Divine` orbit; without it the rule's raw accumulation lands on an
out-of-census fixed point.
...
1. The severed arm's `【` destination recurs at 3x, 3.5x and (from T2.1, saturated) 4x:
