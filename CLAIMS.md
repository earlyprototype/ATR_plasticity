...
| **C-13** | `oja` is **inert** at this site — it moves the basin at no step size tested, up to 5% drift and 100% clip | `supported` | `output_step_size/step_size_map.jsonl` (8 `oja` cells) | One site, one prompt. The likely mechanism (C-14) is not yet tested |
...
| **C-21** | There are **two** ceiling-silent cells that flip the basin, not one | `supported` | `step_size_map.jsonl`: `hebb`@7.07e-05 (1.12% drift) and `hebb`@1.18e-04 (2.20% drift), both 0.0% clip, both `comrade` | **Corrects** the "only cell in the whole sweep" claim in `EXP_001_RESULTS.md` and `HANDOVER.md:96`. This is an independent replication currently being suppressed by an error |
| **C-22** | The basin change is specific to the update's **direction**, not its magnitude | `supported` | Three near-rank-1 updates at matched Frobenius *and* operator norm, all ceiling-silent: `hebb` σ₁ 1.8135 → **flips**; `oja` σ₁ 1.9288 → does not; `anti_hebb` σ₁ 1.8745 → does not. `oja` at σ₁ 4.68 (2.6×) still does not | **Cite this comparison, not the isotropic one.** See C-23 |
