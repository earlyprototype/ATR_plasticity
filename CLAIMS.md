...
| **C-22** | The basin flip requires **both** a sufficient magnitude **and** the right sign on the `hebb`/`oja` axis — neither alone | `provisional` | Loop-state displacement `1−cos(off)`: `hebb`@7.065e-05 **5.000e-03 → flips**; `anti_hebb`@2.944e-05 **4.713e-03 (94%), ceiling-silent, opposite sign → does not flip**; `hebb`@3.925e-05 1.229e-03 (same sign, quarter magnitude) → does not flip | **"Direction, not magnitude" is retired** — F2's α-sweep holds this ΔW's direction fixed and produces three basins by scaling alone (α\*=0.75), and within `hebb` σ₁ 0.9002→`prolet` vs 1.8135→`comrade`. Also **only two directions are sampled**: `oja` = H−D and `anti_hebb` = −H−D with ‖D‖≈110‖H‖, so their loop displacements sit at cos **0.9998** — one axis, two signs. The σ₁-matched triple an earlier draft promoted is **not** a valid control: those cells differ in effect by 24–42×, and `oja` never reaches `hebb`'s perturbation anywhere (max 2.724e-03 vs 5.000e-03). n=1 prompt. **T1.4 is the deciding test** |
...
| **C-50** | Does a rank-1 random edit at matched σ₁ **and matched loop-state displacement** flip the basin? | T1.4 — **can falsify C-22** |
